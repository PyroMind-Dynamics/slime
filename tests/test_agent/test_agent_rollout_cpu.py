...
def _patch_generate(monkeypatch, tokenizer: FakeTokenizer, sandbox_factory) -> None:
    ...
    monkeypatch.setattr(gen, "create_sandbox", lambda image: sandbox_factory(image))  # boot sandbox
    monkeypatch.setattr(swe, "create_sandbox", lambda image: sandbox_factory(image))  # eval sandbox
